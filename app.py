...
# CSS للطباعة (إخفاء كل شيء وإظهار الوصل والستيكر فقط)
st.markdown("""
    <style>
    @media print {
...
# --- 2. البحث والتعديل والطباعة الفورية ---
with tabs[1]:
    ...
    if search_query:
        ...
        for idx, row in results.iterrows():
            with st.expander(f"📋 {row['الزبون']} - {row['الموديل']} (ID: {row['ID']})", expanded=True):
                ...
                # 2. أزرار الطباعة الفورية (خارج الـ form لتعمل فوراً)
                st.write("---")
                st.write("### 🖨️ طباعة فورية")
                
                qr_url = f"https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=https://brhoom-tech.streamlit.app/?search={u_phone}"
                
                # كود HTML المحقون للطباعة
                print_html = f"""
                <div style="display: flex; gap: 10px; margin-bottom: 20px;">
                    <button onclick="window.print()" style="flex: 1; padding: 15px; background-color: #28a745; color: white; border: none; border-radius: 10px; cursor: pointer; font-size: 16px; font-weight: bold;">
                        🖨️ اضغط هنا لفتح نافذة الطباعة فوراً
                    </button>
                </div>
                ...
                <div class="printable">
                    ...
                    <div style="page-break-before: always; margin-top: 20px;" class="sticker-box">
                        <b>{u_name}</b><br>{row['الماركة']} {row['الموديل']}<br>
                        <img src="{qr_url}" width="80"><br>ID: {row['ID']}
                    </div>
                </div>
                """
                components.html(print_html, height=100)
