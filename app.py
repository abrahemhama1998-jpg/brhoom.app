...
st.markdown("""
    ...
    .print-btn {
        background-color: #4CAF50; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; text-decoration: none; display: inline-block; margin: 5px;
    }
    </style>
...
# دالة لتحويل الصورة إلى نص (للتخزين البسيط)
        ...
        for idx, row in results.iterrows():
            with st.expander(f"📋 {row['الزبون']} - {row['الموديل']} (ID: {row['ID']})", expanded=True):
                ...
                with st.form(f"edit_form_{idx}"):
                    ...
                    new_img = st.file_uploader("تحديث صورة الجهاز", type=["jpg", "png", "jpeg"], key=f"img_{idx}")
                    
                    if st.form_submit_button("💾 حفظ التعديلات"):
                        img_data = img_to_base64(new_img) if new_img else row['الصورة']
                        st.session_state.db.loc[idx, ['الزبون', 'الهاتف', 'التكلفة', 'سعر_القطع', 'العطل', 'الحالة', 'الصورة']] = [u_name, u_phone, u_cost, u_parts, u_issue, u_status, img_data]
                        save_data(st.session_state.db)
                        st.success("تم التحديث")
                        st.rerun()

                # --- أزرار الطباعة (خارج الـ form لضمان العمل) ---
                st.write("### 🖨️ أوامر الطباعة")
                
                # إعداد بيانات الباركود للستيكر
                qr_url = f"https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={APP_URL}/?search={u_phone}"
                
                # كود HTML للوصل والستيكر المخفيين
                st.markdown(f"""
                <div id="print_rec_{idx}" class="printable receipt-box">
                    <h1 style="text-align:center;">الحل للتقنية</h1>
                    <p style="text-align:center;">تواصل: 0916206100</p><hr>
                    <p><b>الرقم:</b> {row['ID']} | <b>الزبون:</b> {u_name}</p>
                    <p><b>الجهاز:</b> {row['الماركة']} {u_model if 'u_model' in locals() else row['الموديل']}</p>
                    <p><b>التكلفة:</b> {u_cost} $</p>
                    <p><b>التاريخ:</b> {row['التاريخ']}</p>
                </div>
                <div id="print_stk_{idx}" class="printable sticker-box">
                ...
                c_p1.button("📑 فتح نافذة طباعة الوصل", on_click=lambda: st.js_code("window.print()"), key=f"btn_p1_{idx}")
                c_p2.button("🏷️ فتح نافذة طباعة الستيكر", on_click=lambda: st.js_code("window.print()"), key=f"btn_p2_{idx}")
                
                st.info("💡 ملاحظة: بعد الضغط على الزر، استخدم Ctrl+P (كمبيوتر) أو خيار الطباعة (جوال).")
...
with tabs[2]:
    ...
    st.dataframe(st.session_state.db.drop(columns=['الصورة'])) # عدم عرض نص الصورة الطويل
